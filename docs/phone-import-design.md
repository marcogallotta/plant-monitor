...
**Phase 0 — Confirm the seam (no UI change).**
Verify `/manual-photos` accepts and persists `source: phone` end-to-end (client already passes it via `buildUploadFormData`; `internals.md:78` implies it may still hardcode `manual` — check and fix if so). Add/confirm a backend test asserting a `source=phone` upload stores `phone`. Nothing user-facing; this unblocks every later phase and any future v2 query that keys on `source`.

**Phase 1 — Grid with immediate byte capture (the actual bug fix).**
Replace the current phone file-picker flow with a thumbnail grid. On selection, for each file: **read original bytes promptly into the upload Blob**, and **decode once to 128px** for the thumbnail → drop the original `File` reference (the "Stale-`File` safety" lifecycle — two separate captures; upload the original bytes, never the 128px re-encode). Bounded decode concurrency (~3–4 `createImageBitmap`, `close()` after draw); chunk into batches of 20–40; revoke object URLs on clear. **No scoring, no ordering yet** — default selection all-selected, natural picker order. Show **per-photo state immediately — `queued / ready / failed-to-read`** — surfacing read failures the instant they happen, since stale-handle/read errors are the exact bug this phase fixes (a silent failure here would defeat the point). This phase alone kills the stale-`File` failures and is a complete, useful feature.
